(function(){
"use strict";
const p=new URLSearchParams(location.search),req=p.get("hubLang")||p.get("lang")||localStorage.getItem("bio-radar-language"),lang=req==="en"?"en":"es";
try{localStorage.setItem("bio-radar-language",lang)}catch(e){}
document.documentElement.lang=lang;
const pairs=[...(window.BR_ES_1||[]),...(window.BR_ES_2||[]),...(window.BR_ES_3||[]),...(window.BR_ES_4||[])];
...
function control(){
  if(document.getElementById("br-language"))return;
  const b=document.createElement("div");b.id="br-language";
  b.setAttribute("role","group");b.setAttribute("aria-label",lang==="es"?"Idioma":"Language");
  ...
  b.querySelectorAll("button").forEach(x=>{
    x.type="button";x.style.cssText="border:0;background:transparent;color:#fff;font:inherit;cursor:pointer;padding:2px 4px";
...
function init(){
  ...
  if(lang==="es"){
    ...
    document.addEventListener("click",ev=>{
      ...
      const u=new SpeechSynthesisUtterance(msg);u.lang="es-ES";u.rate=.95;
